...
#endif
{
    ...
    mLayerMenu->addAction(mActionHandler->actionSelectPreviousLevel());
    mLayerMenu->addAction(mActionHandler->actionSelectNextLevel());
...
void MainWindow::convertToLot()
{
    ...
    if (map->orientation() == Map::LevelIsometric)
        bounds.translate(mMapDocument->currentLevelIndex() * QPoint(-3, -3));
    ...
    QScopedPointer<Map> clone(new Map(mapOrient, mapWidth, mapHeight, map->tileWidth(), map->tileHeight()));
    for (Tileset *ts : map->tilesets())
        clone->addTileset(ts);
    ...
    const QList<Layer*> layers = map->layers();
    ...
    TmxMapWriter writer;
    if (!writer.write(clone.data(), dialog.filePath())) {
        QMessageBox::critical(this, tr("Error Saving Map"),
                              writer.errorString());
...
void MainWindow::RoomDefGo()
{
    ...
    for (int level = 0; level <= mMapDocument->mapComposite()->maxLevel(); level++) {
        ...
        ObjectGroup *og = nullptr;
        if (index < 0) {
            // Create the new layer in the same level as the current layer.
            // Stack it with other layers of the same type in level-order.
            index = mapLevel->layerCount();
            Layer *topLayerOfSameTypeInSameLevel = nullptr;
            Layer *bottomLayerOfSameTypeInGreaterLevel = nullptr;
            Layer *topLayerOfSameTypeInLesserLevel = nullptr;
            const QList<Layer*> layers = mapLevel->layers(Layer::ObjectGroupType);
            for (Layer *layer : layers) {
            ...
            if (topLayerOfSameTypeInSameLevel)
                index = mapLevel->layers().indexOf(topLayerOfSameTypeInSameLevel) + 1;
            else if (bottomLayerOfSameTypeInGreaterLevel)
                index = mapLevel->layers().indexOf(bottomLayerOfSameTypeInGreaterLevel);
            else if (topLayerOfSameTypeInLesserLevel)
                index = mapLevel->layers().indexOf(topLayerOfSameTypeInLesserLevel) + 1;
            og = new ObjectGroup(layerName, 0, 0, map->width(), map->height());
            og->setColor(Qt::blue);
            og->setLevel(level);
            mMapDocument->undoStack()->push(new AddLayer(mMapDocument, level, index, og));
        } else {
            og = mapLevel->layerAt(index)->asObjectGroup();
            mMapDocument->setLayerVisible(level, index, true);
        }
...
void MainWindow::RoomDefMerge()
{
    ...
    if (index < 0)
        return;
    ObjectGroup *og = mapLevel->layerAt(index)->asObjectGroup();
...
void MainWindow::RoomDefRemove()
{
    ...
    for (MapObject *o : qAsConst(remove))
...
void MainWindow::RoomDefUnknownWalls()
{
    ...
    RoomDefecator rd(mMapDocument->map(), mMapDocument->currentLevelIndex(), bounds);
    if (!rd.mLayerWalls)
        return;
...
private:
    void swap()
    {
        ...
        mLayerIndex = reorderLayer(mLevelIndex, mLayerIndex);
    ...
    int reorderLayer(int levelIndex, int index)
    {
        Q_UNUSED(index)
        MapLevel *mapLevel = mDocument->map()->levelAt(levelIndex);
        int old = mapLevel->layers().indexOf(mLayer);
        LayerModel *layerModel = mDocument->layerModel();
        Layer *layer = layerModel->takeLayerAt(levelIndex, old);
...
void MainWindow::ApplyScriptChanges(MapDocument *doc, const QString &undoText, Lua::LuaMap *mMap)
{
    ...
    // Tilesets added.
    for (Tileset *lts : qAsConst(mMap->mNewTilesets)) {
        bool found = false;
        for (Tileset *ts : doc->map()->tilesets()) {
            if (ts->name() == lts->name()) {
                found = true;
                break;
            }
...
#ifdef ZOMBOID
    MapLevel *mapLevel = mMapDocument ? mMapDocument->currentMapLevel() : nullptr;
    if (mapLevel == nullptr) {
        ...
        mCurrentLevelButton->setText(tr("Level: %1 ").arg(mapLevel->z()));
    }
    if (layer) {
...
void MainWindow::aboutToShowLevelMenu()
{
    ...
    QStringList items;
    for (int z = mMapDocument->map()->levelCount() - 1; z >= 0; z--) {
        QAction *action = mCurrentLevelMenu->addAction(QString::number(z));
        if (z == mMapDocument->currentLevelIndex()) {
...
void MainWindow::aboutToShowLayerMenu()
{
    ...
    const QList<TileLayer*> tileLayers = mapLevel->tileLayers();
    ...
    const QList<ObjectGroup*> objectGroups = mapLevel->objectGroups();
...
#endif
    mMapDocument->setCurrentLayerIndex(level, mapLevel->layerCount() > 0 ? 0 : -1);
}
